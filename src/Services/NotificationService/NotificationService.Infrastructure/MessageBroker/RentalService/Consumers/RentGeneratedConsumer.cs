...
public class RentGeneratedConsumer : BackgroundService
{
    ...
    public RentGeneratedConsumer(IServiceScopeFactory serviceScopeFactory, IOptions<RabbitMQSettiings> settings)
    {
        _settings = settings;
        ...
        _queueName = _settings.Value.Queues["RentalCreated"]!;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        _consumer.Received += async (model, ea) =>
        {
            ...
            try
            {
                var rentGeneratedEvent = JsonConvert.DeserializeObject<RentGeneratedDomainEvent>(message);
                if (rentGeneratedEvent == null)
                {
                ...
                using (var scope = _serviceScopeFactory.CreateScope())
                {
                    ...
                    await publisher.Publish((INotification)rentGeneratedEvent);
                }

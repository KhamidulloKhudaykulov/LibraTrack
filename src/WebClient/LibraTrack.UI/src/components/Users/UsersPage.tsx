    ...
    return (
        <div className="bg-white w-full rounded-xl flex flex-col h-auto flex-1">
            ...
            <div className="flex flex-row border-t p-4 border-b border-gray-200 font-bold text-gray-400">
                <h2 className="flex-1 items-center">Пасспорт</h2>
                <h2 className="flex-1 items-center">Пользователь</h2>
                <h2 className="flex-1 items-center">Номер телефона</h2>
                <h2 className="flex-1 items-center">Эл.почта</h2>
                <h2 className="flex-1 items-center">Статус</h2>
                <h2 className="flex-1 items-center">Действии</h2>
            </div>
            {loading &&
            ...
            {filteredUsers.length === 0 && (
                <p className="ml-auto mr-auto text-gray-400 text-sm">Нет данных</p>
            )}
            {filteredUsers.map((user) => (
                <div
                    key={user.id}
                    className="flex flex-row p-4 border-b py-6 hover:bg-gray-50 border-gray-200 text-gray-700 cursor-default">
                    <p className="flex-1 truncate text-blue-500 font-bold">
                        <NavLink to={""} 
                    ...
                    <div className="flex-1">
                        <button
                            ...
                            onClick={() => { setShowEditModal(true); setSelectedUser(user); }}>Редактировать</button>
                        <button className="text-red-500 hover:underline mr-2 font-bold cursor-pointer"
                            onClick={() => handleBlock(user.id)}>Блокировать</button>
                        <button className="text-green-500 hover:underline font-bold cursor-pointer"
                            onClick={() => handleActive(user.id)}>Активировать</button>
                    </div>
                </div>
            ))}
